...
void PrintHotDogResult(const Result<HotDog>& result,
    ...
    if (result.HasValue()) {
        ...
        os << "Hot dog #" << hot_dog.GetId() << ": bread bake time: "
           << as_seconds(hot_dog.GetBread().GetBakingDuration())
           << "s, sausage bake time: "
           << as_seconds(hot_dog.GetSausage().GetCookDuration()) << "s"
           << std::endl;
    } else {
        try {
...
std::vector<HotDog> PrepareHotDogs(int num_orders, unsigned num_threads) {
    ...
    // std::latch - объект синхронизации, который ведёт себя как счётчик
    // обратного отсчёта. num_waiting_threads потоков будут ожидать, пока этот
    // счётчик не обнулится
    std::latch start{num_waiting_threads};

    for (int i = 0; i < num_orders; ++i) {
        // Выполняем функцию через boost::asio::dispatch, чтобы вызвать
        // Cafeteria::OrderHotDog в нескольких потоках
        net::dispatch(io, [&cafeteria, &hotdogs, &mut, i, start_time, &start,
                           num_waiting_threads] {
            std::osyncstream{std::cout}
                << "Order #" << i << " is scheduled on thread #"
                << std::this_thread::get_id() << std::endl;

            // Ждём, пока все рабочие потоки зайдут в эту функцию, чтобы
            // убедиться, что OrderHotDog будет вызван из нескольких потоков
...
void VerifyHotDogs(const std::vector<HotDog>& hotdogs) {
    ...
    for (auto& hotdog : hotdogs) {
        ...
        {
            auto [_, sausage_id_is_unique] =
                sausage_ids.insert(hotdog.GetSausage().GetId());
            assert(sausage_id_is_unique);
        }

        // Хлеб должен иметь уникальный id
        {
            auto [_, bread_id_is_unique] =
                bread_ids.insert(hotdog.GetBread().GetId());
...
int main() {
    ...
    std::cout << "Cook duration: "
              << duration_cast<duration<double>>(cook_duration).count() << 's'
              << std::endl;

    // Все заказы должны быть выполнены
    assert(hotdogs.size() == num_orders);
    // Ожидаемое время приготовления 20 хот-догов на 4 рабочих потоках: от 7
    // до 7.5 секунд
    //
