...
// Вспомогательный класс Result, способный хранить либо значение, либо
// исключение
...
class Result {
   public:
     ...
     *
     * Result<тип> result{std::make_exception_ptr(std::runtime_error{"some
     * error"})};
     */
    ...
    /*
     * Создаёт результат, который хранит ссылку на текущее выброшенное
     * исключение Способ использования: try { <код, который может выбросить
     * исключение> } catch (...) { auto result =
     * Result<тип>::FromCurrentException();
     * }
     */
    static Result FromCurrentException() { return std::current_exception(); }
    ...
    // Если внутри содержится исключение, то выбрасывает его. Иначе не делает
    // ничего.
    ...
    // Возвращает ссылку на хранящееся значение. Если Result хранит исключение,
    // выбрасывает std::bad_variant_access
    const ValueType& GetValue() const& { return std::get<ValueType>(state_); }

    // Возвращает rvalue-ссылку на хранящееся значение. Если Result хранит
    // исключение, выбрасывает std::bad_variant_access
    ValueType&& GetValue() && { return std::get<ValueType>(std::move(state_)); }
