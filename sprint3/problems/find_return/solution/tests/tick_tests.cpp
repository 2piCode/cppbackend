...
SCENARIO("Tick tests") {
    GIVEN("empty players and empty game") {
        ...
        AND_GIVEN("empty game") {
            std::shared_ptr<EmptyGame> game =
                std::make_shared<EmptyGame>(app::Game::Maps{}, 10.0);
            GameTickUseCase use_case(game, players, nullptr, nullptr);
        ...
        AND_GIVEN("game with one map") {
            std::shared_ptr<GameWithOneMap> game =
                std::make_shared<GameWithOneMap>(app::Game::Maps{}, 10.0);
