...
SCENARIO("Get map tests") {
    GIVEN("game with zero map") {
        app::Game::Pointer game =
            std::make_shared<EmptyGame>(app::Game::Maps{}, 1.0);
    ...
    GIVEN("game with correct map") {
        std::shared_ptr<GameWithOneMap> game =
            std::make_shared<GameWithOneMap>(app::Game::Maps{}, 1.0);

        AND_GIVEN("empty loot handler") {
            GetMapUseCase use_case(game, std::make_unique<LootHandler>(
        ...
        AND_GIVEN("loot types array with map") {
            ...
            GetMapUseCase use_case(
                game, std::make_unique<LootHandler>(LootHandler::LootTypeByMap{
