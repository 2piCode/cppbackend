   ...
   private:
    ...
    void MovePlayers(std::chrono::milliseconds delta_time) {
        ...
        std::unordered_map<size_t,
                           std::variant<game::Item::Id, const model::Office*>>
            item_id_item;
        ...
        for (const auto& [map, _] : map_gatherers) {
            ...
            for (const auto& item : game_session->GetLootPositionsInfo()) {
                map_items[map].push_back(collision_detector::Item{
                    .position = item.position, .width = game::ItemWidth / 2});
    ...
    void ProcessEvents(
        ...
        const std::unordered_map<
            size_t, std::variant<game::Item::Id, const model::Office*>>&
            item_id_item,
        ...
        for (const auto& event : events) {
            ...
            if (auto item_ptr = std::get_if<const model::Office*>(&item)) {
                player->DropAllItems();
            } else if (auto item_ptr = std::get_if<game::Item::Id>(&item)) {
