...
class Controller {
   public:
    Controller(TV& tv, Menu& menu) : tv_{tv}, menu_{menu} {
        ...
        menu_.AddAction(std::string{INFO_COMMAND}, {},
   ...
   private:
     ...
     * Если в input содержатся какие-либо параметры, выводит сообщение об
     * ошибке: Error: the Info command does not require any arguments
     */
    [[nodiscard]] bool ShowInfo(std::istream& input,
        ...
        if (EnsureNoArgsInInput(INFO_COMMAND, input, output)) {
            if (tv_.IsTurnedOn()) {
                ...
                output << "Channel number is " << *tv_.GetChannel()
                       << std::endl;
            } else {
     ...
     * Если в input содержатся какие-либо параметры, не включает телевизор и
     * выводит сообщение об ошибке: Error: the TurnOff command does not require
     * any arguments
     */
     ...
     * Если в input содержатся какие-либо параметры, не выключает телевизор и
     * выводит сообщение об ошибке: Error: the TurnOff command does not require
     * any arguments
     */
    [[nodiscard]] bool TurnOff(std::istream& input,
                               std::ostream& output) const {
        using namespace std::literals;
     ...
     * Если номер канала - не целое число, выводит в output ошибку "Invalid
     * channel" Обрабатывает ошибки переключения каналов на телевизор и выводит
     * в output сообщения:
     * - "Channel is out of range", если TV::SelectChannel выбросил
     * std::out_of_range
    ...
    [[nodiscard]] bool SelectChannel(std::istream& input,
        ...
        output << "Error: the " << SELECT_CHANNEL_COMMAND
               << " command requires channel number argument" << std::endl;
        return true;
    }

    /*
     * Обрабатывает команду SelectPreviousChannel, выбирая предыдущий канал на
     * tv_. Если TV::SelectLastViewedChannel выбросил std::logic_error, выводит
     * в output сообщение: "TV is turned off"
     */
    [[nodiscard]] bool SelectPreviousChannel(std::istream& input,
                                             std::ostream& output) const {
        if (EnsureNoArgsInInput(SELECT_PREVIOUS_CHANNEL_COMMAND, input,
                                output)) {
            try {
