...
#include "player_action_request.h"
...
ApiHandler::ApiHandler(app::Game::Pointer game) : app_{game} {
    InitializeRoutes();
}
...
ApiHandler::StringResponse ApiHandler::GetMapResponse(
    ...
    try {
        return response_utils::MakeOkResponseCached(
            json_converter::FullMapToJson(*app_.GetMap(map_id)));
...
ApiHandler::StringResponse ApiHandler::GameJoin(const http::verb method,
    ...
    try {
        auto result = app_.JoinGame(model::Map::Id{join_game_request->map_id},
        ...
        authorization_header, [this](const app::Token& token) {
            try {
                return response_utils::MakeOkResponse(
                    json_converter::GameStateToJson(app_.GetGameState(token)));
    ...
    return ExecuteAuthorized(
        authorization_header, [this, &body](const app::Token& token) {
            try {
                ...
                app_.MovePlayer(token, request->direction);
                return response_utils::MakeOkResponse({});
