...
struct MapFields {
    ...
    constexpr static boost::string_view LOOT_TYPES = "lootTypes";
};
...
struct GameStateFields {
    ...
    constexpr static boost::string_view LOST_OBJECTS = "lostObjects";
};
...
struct PlayerGameStateFields {
    ...
    constexpr static boost::string_view BAG = "bag";
    constexpr static boost::string_view SCORE = "score";
};

struct GameRecordFields {
    GameRecordFields() = delete;
    constexpr static boost::string_view NAME = "name";
    constexpr static boost::string_view SCORE = "score";
    constexpr static boost::string_view PLAY_TIME = "playTime";
};

struct ItemFields {
    constexpr static boost::string_view ID = "id";
    constexpr static boost::string_view TYPE = "type";
    constexpr static boost::string_view POS = "pos";
};
...
boost::json::object json_converter::ItemToJson(model::Item item_info) {
    return boost::json::object{
        {ItemFields::TYPE, item_info.type},
        {ItemFields::POS,
         boost::json::array{item_info.position.x, item_info.position.y}}};
}
...
boost::json::object json_converter::GameStateToJson(
    ...
    boost::json::object lost_object;
    for (size_t i = 0; i < game_state.lost_objects.size(); i++) {
        lost_object[std::to_string(i)] = ItemToJson(game_state.lost_objects[i]);
    }

    main[GameStateFields::PLAYERS] = players;
    main[GameStateFields::LOST_OBJECTS] = lost_object;
...
boost::json::object json_converter::PlayerGameStateToJson(
    ...
    for (const auto& item : player_game_state.items) {
        bag_array.push_back(boost::json::object{{ItemFields::ID, *item.id},
    ...
    return boost::json::object{
        ...
        {PlayerGameStateFields::BAG, bag_array},
        {PlayerGameStateFields::SCORE, player_game_state.score}};
}

boost::json::object json_converter::GameRecordToJson(
    const GameRecord& game_record) {
    return boost::json::object{
        {GameRecordFields::NAME, game_record.name},
        {GameRecordFields::SCORE, game_record.score},
        {GameRecordFields::PLAY_TIME, game_record.time_in_game}};
}

model::Map::Pointer json_converter::JsonToMap(const json::object map_json) {
    std::optional<double> dog_speed = std::nullopt;
    if (auto dog_speed_it = map_json.find(MapFields::DOG_SPEED);
    ...
    int number_of_loot_types =
        map_json.at(MapFields::LOOT_TYPES).as_array().size() - 1;
    ...
    return std::make_shared<model::Map>(
        ...
        offices, dog_speed, number_of_loot_types);
}
