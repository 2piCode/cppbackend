...
class View {
   ...
   private:
    ...
    std::optional<detail::AuthorInfo> AuthorExists(
    ...
    std::optional<detail::BookInfo> SelectBook(
        const std::vector<detail::BookInfo>&) const;
    std::optional<detail::BookInfo> SelectBookAnyWay(
        std::istream& cmd_input) const;
    void PrintBook(const detail::BookInfo&,
